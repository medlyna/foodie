...
$('#ingredient_search').on("submit",function(evt){
  ...
  var ingredient = document.getElementById("pre-selected-options").value;
  var allergy = document.getElementById("pre-selected-options_allergy").value;

  alert('https://api.yummly.com/v1/api/recipes?_app_id=4dff1881&_app_key=6a53d93644cf7dd923a5b67c226c20f1&q='+ingredient+'+allowedAllergy[]='+allergy);
	var url = 'https://api.yummly.com/v1/api/recipes?_app_id=4dff1881&_app_key=6a53d93644cf7dd923a5b67c226c20f1&q='+ingredient+'allowedAllergy[]='+allergy;
		$.ajax({
			url: url,
			dataType: 'json',
		...
		// console.log(parsed_json.matches)
		 for (i=0; i<parsed_json.matches.length; i++){
		 	console.log(parsed_json.matches[i].recipeName)
		 	 var query = encodeURIComponent(parsed_json.matches[i].recipeName);
		 	 var link = 'https://google.com/?#q=' + query;
		 	 console.log(query)
			$(".recipe-list").append(
			"<li><a href="+ link +">" + parsed_json.matches[i].recipeName + "</a></li>"); 	
			}
		}
});
